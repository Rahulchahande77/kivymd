...
class MDDatePicker(FloatLayout, ThemableBehavior, RectangularElevationBehavior,
    ...
    def fmt_lbl_date(self, year, month, day, orientation):
        ...
        try:
            ...
            return d.strftime('%a,').capitalize() + separator + d.strftime('%b').capitalize() + ' ' + str(day).lstrip('0')
        except Exception as e:
            print("go yr month day", int(year), int(month), int(day))
            print("Exception while fmt_lbl_date error ",str(e))
        ...
        try:
            ...
            for idx in range(len(self.cal_list)):
                if idx >= len(dates) or dates[idx].month != month:
                    ...
                    self.cal_list[idx].disabled = False if str(self.to_date) <= str(dates[idx])>= str(self.from_date) else True
