...
[FileThumbEntry@Widget]:
    ...
    canvas:
        Color:
            rgba: 0, 0, 0, 1 if self.selected else 0
        BorderImage:
            border: 8, 8, 8, 8
            pos: root.pos
            size:dp(100),dp(120)
    ...
    AsyncImage:
        id: image
        # size:dp(90),dp(90)
        size: ctx.controller().thumbsize - dp(50), ctx.controller().thumbsize - dp(50)
        pos: root.center_x - self.width / 2, root.y + dp(30)
    Label:
        text: ctx.name
        color:0,0,0,1
...
DEFAULT_THEME = 'atlas://kivymd/images/filebrowser_atlas/'
