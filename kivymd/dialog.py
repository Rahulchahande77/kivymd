...
class MDDialog(ThemableBehavior, RectangularElevationBehavior, ModalView):
    ...
    def add_action_button(self, text,action=None,**kwargs ):
        ...
        def check_validation(content,*args):
            ...
            try:
                for child in content.children:
                    if isinstance(child, MDSelect) or isinstance(child, MDTextField):
                        if child.error:
                            self.invalid = True
                            continue
                        else:
        ...
        if kwargs.get('validate') and kwargs.get('content'):
            ...
            self.validation_clock=Clock.schedule_interval(partial(check_validation, kwargs.get('content')), 0.5)
